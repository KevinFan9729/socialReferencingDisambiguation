...
def segmentation(img, bb):
    #dcrease brightness and increase contrast.
    ####################
    # cv2.imwrite('segmented.png',img)
    # alpha = 1.2
    # beta = 85
    # alpha_arr = np.full((img.shape), alpha, dtype= 'uint8')
    # img = cv2.multiply(img, alpha_arr)
    # beta_arr = np.full((img.shape), beta, dtype= 'uint8')
    # img = cv2.subtract(img,beta_arr)
    ####################################
...
class YoloDetector():
    ...
    def get_item_3d_coordinates(self,i_name, rgb_img):#3d coordinates based on the base link
        ...
        for k, [item_name, box] in enumerate(zip(self.class_names, self.boxes)):
            ...
            # if i_name =="cup":
            #     cv2.cv2.imwrite("cup_mask.png", mask)
            pc = []
    ...
    def get_item_3d_coordinates_base_map(self,i_name, rgb_img):#3d coordinates based on the base frame and the map frame
        c, u = [], []
        for k, [item_name, box] in enumerate(zip(self.class_names, self.boxes)):
            if not (i_name==item_name):
                continue
            ...
            w, h = box[2] - box[0], box[3] - box[1]
            for pix in object_loc:
                temp=self._2dto3d.pixelTo3DPointBaseMap(pix[1], pix[0])
                # temp=np.array(temp)
                # print(temp.shape)
                if temp!=None:
                    _c = temp[0]#base
            ...
            coord_3d_cam = [coord_3d_cam[0], coord_3d_cam[1], np.percentile(pc[:,2], 75)]
            coord_3d_map = [coord_3d_map[0], coord_3d_map[1], np.percentile(pc[:,2], 75)]
            # up_3d = [coord_3d[0], coord_3d[1], np.percentile(pc[:,2], 95)]
            ...
            #     u.append(coord_3d_map)
                ...
                #c is the object coordinates in the base frame
...
class Get3Dcoordinates(object):
    ...
    def pixelTo3DPoint(self, u, v, cloud=None):
        ...
        #this is important from map to base_link
    ...
    ## transform 2D point into 3D coordinates in the "base" frame and "map" frame
    def pixelTo3DPointBaseMap(self, u, v, cloud=None):
        gen = pc2.read_points(self.cloud, field_names=("x", "y", "z"), skip_nans=True, uvs=[[int(u), int(v)]])
        try:
            target_xyz_cam = list(gen)
        except:
        ...
        return [[base_point.point.x, base_point.point.y, base_point.point.z],[map_point.point.x, map_point.point.y, map_point.point.z]]
